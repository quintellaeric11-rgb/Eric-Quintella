...
const appUrl=(process.env.KONKI_APP_URL||'http://localhost:3000').replace(/\/$/,'');
...
try{
  ...
  const conquestId=ok(await youth.c.rpc('create_conquest',{conquest_title:'Quero comprar um tênis',conquest_category:'PURCHASE',conquest_reason:'Porque estou precisando'}),'criar conquista');
  const conquest=ok(await youth.c.from('conquests').select('*').eq('id',conquestId).single(),'ler conquista');
  const response=await fetch(`${appUrl}/api/missions/recommend`,{method:'POST',headers:{'content-type':'application/json',authorization:`Bearer ${youth.token}`},body:JSON.stringify({conquestId:conquest.id})});
  ...
  const reviewNotice=ok(await parent.c.from('notifications').select('*').eq('type','JOURNEY_REVIEW').eq('related_entity_id',journey.id).single(),'notificação imediata ao Parent');assert.equal(reviewNotice.deep_link,`/?view=journey-review&journey=${journey.id}`);assert(reviewNotice.title.includes('criou uma nova conquista'));
  ...
  ok(await parent.c.from('journey_missions').update({effort_weight:5,title:'Comparar três opções de tênis',estimated_minutes:45,xp_reward:175,evidence_types:['TEXT','IMAGE'],skills:['Educação financeira','Tomada de decisão']}).eq('id',missions[0].id),'editar todos os detalhes permitidos');
  ...
  const firstBefore=missions[0],secondBefore=missions[1];ok(await parent.c.from('journey_missions').update({mission_order:1000+firstBefore.mission_order}).eq('id',firstBefore.id),'reordenar temporário');ok(await parent.c.from('journey_missions').update({mission_order:firstBefore.mission_order}).eq('id',secondBefore.id),'reordenar segundo');ok(await parent.c.from('journey_missions').update({mission_order:secondBefore.mission_order}).eq('id',firstBefore.id),'reordenar primeiro');
  missions=ok(await parent.c.from('journey_missions').select('*').eq('journey_id',journey.id).order('mission_order'),'persistir nova ordem');assert.equal(missions[0].id,secondBefore.id);assert.equal(sum(missions),100);
  const custom=ok(await parent.c.from('journey_missions').insert({journey_id:journey.id,family_id:family.id,title:'Experimentar e registrar o ajuste do tênis',description:'Compare conforto e tamanho antes de decidir.',instructions:'Experimente duas opções e registre os critérios da escolha.',category:'DEVELOPMENT_CONTEXTUALIZED',skills:['Autonomia','Tomada de decisão'],estimated_minutes:30,xp_reward:150,goal_progress_reward:0,evidence_types:['TEXT','IMAGE'],phase:3,mission_order:99,recommendation_reasons:['Criada pelo responsável'],why_this_mission:'Conecta a decisão de compra a uma necessidade concreta.',effort_weight:3,is_custom:true}).select().single(),'adicionar missão');
  ...
  const desiredDate=new Date(Date.now()+120*86400000).toISOString().slice(0,10);
  const approved=ok(await parent.c.rpc('approve_journey_with_deadline',{target_journey:journey.id,goal_value:650,reward_text:'Cumprir o combinado para a compra do tênis.',duration_months:3,goal_deadline:desiredDate}),'aprovar jornada');assert.equal(approved.status,'APPROVED');
  const persisted=ok(await parent.c.from('conquests').select('desired_date,confirmed_goal_value,reward_agreement').eq('id',conquest.id).single(),'persistência do combinado');assert.equal(persisted.desired_date,desiredDate);assert.equal(Number(persisted.confirmed_goal_value),650);
  ...
  const active=ok(await youth.c.from('mission_assignments').select('*,mission:missions(*)').eq('conquest_id',conquest.id).order('mission_order',{nullsFirst:false}),'jovem vê missões'),sequential=active.filter(a=>a.journey_id),independent=active.filter(a=>!a.journey_id);assert.equal(active.length,missions.length);assert.equal(sequential[0].status,'AVAILABLE');assert(sequential.slice(1).every(a=>a.status==='LOCKED'),'somente a primeira missão KONKI deve estar disponível');assert.equal(independent.length,1);assert.equal(independent[0].status,'AVAILABLE');assert.equal(independent[0].mission_order,null);assert.deepEqual(sequential.map(a=>a.mission_order),missions.filter(m=>!m.is_custom).map(m=>m.mission_order));assert.equal(sequential[0].mission.lesson_content.source_type,'KONKI');assert.equal(independent[0].mission.lesson_content.source_type,'PARENT_CUSTOM');assert.equal(independent[0].mission.lesson_content.microaula,null);
  ...
  const sequenceAfterApproval=ok(await youth.c.from('mission_assignments').select('id,status,mission_order,journey_id').eq('conquest_id',conquest.id).not('journey_id','is',null).order('mission_order'),'próxima missão');assert.equal(sequenceAfterApproval[0].status,'APPROVED');assert.equal(sequenceAfterApproval[1].status,'AVAILABLE');assert(sequenceAfterApproval.slice(2).every(a=>a.status==='LOCKED'));
  ...
  console.log(`PASS: fluxo dedicado tênis em ${appUrl}; ${missions.length} missões, progresso ponderado 100%, edição/remoção/adição e combinado persistidos, aceite Parent e Youth separado, evidência aprovada e RLS entre duas famílias.`);
