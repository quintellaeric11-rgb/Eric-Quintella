-- Complete the remaining second-update flows without changing Waves 1/2 or the mission engine.
...
begin
  ...
  loop
    invite_code:=upper(substr(encode(gen_random_bytes(8),'hex'),1,6));
...
create or replace function public.update_wishlist_item(target_item uuid,item_title text,item_context text default null) returns void
language plpgsql security definer set search_path=public,pg_temp as $$
declare caller uuid:=auth.uid(); item public.conquest_wishlist; safe_title text;
begin
  select * into item from public.conquest_wishlist where id=target_item for update;
  safe_title:=nullif(btrim(item_title),'');
  if item.id is null or item.youth_id<>caller or item.status<>'SAVED' or safe_title is null or char_length(safe_title)>160 then raise exception 'not_allowed'; end if;
  if not exists(select 1 from public.family_members where family_id=item.family_id and profile_id=caller and role='YOUTH') then raise exception 'not_allowed'; end if;
  update public.conquest_wishlist set title=safe_title,context=nullif(btrim(item_context),''),updated_at=now() where id=item.id;
end $$;
...
revoke all on function public.update_wishlist_item(uuid,text,text) from public,anon;
grant execute on function public.update_wishlist_item(uuid,text,text) to authenticated,service_role;
