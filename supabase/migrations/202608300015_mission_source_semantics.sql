-- Preserve the distinction between KONKI pedagogy and a Parent-authored task.
-- Incremental replacement of the current approval RPC; no historical data is removed.
create or replace function public.approve_journey_with_agreement(target_journey uuid,goal_value numeric default null,reward_text text default null,duration_months integer default 3) returns jsonb
language plpgsql security definer set search_path=public,pg_temp as $$
declare journey public.journeys; conquest public.conquests; planned public.journey_missions; parent_id uuid; mission_id uuid; total integer; sequence integer:=0;
begin
  ...
  for planned in select * from public.journey_missions where journey_id=journey.id order by mission_order,id loop
    ...
    values('family-'||journey.family_id||'-'||planned.id,planned.title,coalesce(planned.contextualized_intro,planned.description),planned.category,planned.skills,'{}',array[conquest.category],8,21,'MEDIUM',planned.estimated_minutes,planned.xp_reward,planned.progress_percentage,planned.evidence_types,case when planned.is_custom then 'Orientação do responsável' else 'Antes de começar' end,jsonb_build_object(
      'source_type',case when planned.is_custom then 'PARENT_CUSTOM' else 'KONKI' end,
      'contexto',planned.contextualized_intro,'microaula',planned.contextualized_micro_lesson,'passos',planned.contextualized_steps,
      'prova',coalesce(planned.contextualized_evidence_request,'Envie o resultado combinado com seu responsável.'),'follow_up',planned.follow_up_question,'age_band',planned.pedagogical_age_band,
  ...
  update public.conquests set status='APPROVED',approved_at=now(),confirmed_goal_value=goal_value,reward_agreement=reward_text,estimated_duration_months=duration_months,recommended_mission_count=total where id=journey.conquest_id;
