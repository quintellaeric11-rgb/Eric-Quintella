...
function adaptPedagogy({age,classification,archetype,baseSteps}){
  ...
  if(band==='12')return{
    ...
    parent_support:`O responsável pode ajudar a encontrar duas fontes, fazer perguntas curtas e revisar o resultado, sem escolher pelo jovem.`,
  ...
  if(band==='14')return{
    ...
    steps:[`Defina o resultado que precisa alcançar nesta missão.`,...(baseSteps.slice(0,2)),`Compare pelo menos três alternativas usando os mesmos critérios.`,`Escolha uma alternativa e justifique com dados; peça ao responsável uma pergunta crítica antes de finalizar.`],
  ...
  return{
    ...
    parent_support:`O responsável informa restrições reais da família e atua como contraponto na defesa final; pesquisa, análise e recomendação ficam com o jovem.`,
    reflection:`O que você aceitou abrir mão? Qual suposição é mais frágil e como a decisão mudaria em outro cenário?`,
    evidence:`Entregue uma análise com fontes, critérios e pesos, cenário alternativo, riscos e uma defesa final da decisão.`,
