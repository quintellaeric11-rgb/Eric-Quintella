...
const patterns=[
  {test:/(?:organizar|criar|produzir|montar)\s+(?:um\s+|uma\s+|o\s+|a\s+)?(?:evento|show|festival)/i,type:'project',category:'event_project',natural:()=>`o evento que você quer realizar`,complexity:'very_high',components:{financial:'medium',planning:'high',research:'medium',logistics:'high',social:'high',pedagogical:'very_high'}},
...
function ageTitle(code,base,c,band){
  ...
  const older={CQ02:`Defina requisitos, limites e prioridades para ${c.natural_reference}`,CQ03:`Priorize critérios e entenda o que ganha e abre mão`,CQ01:`Analise custo total e custo de oportunidade`,DC01:`Compare alternativas com critérios ponderados`,DC08:`Construa uma matriz de decisão`,CQ04:`Estabeleça um limite e justifique a escolha`,CQ05:`Investigue custos indiretos e riscos`,CQ11:`Defenda a decisão final com evidências`,CQ12:`Planeje a execução e antecipe riscos`};
...
function missionActivity(code,reference){
 if(['DC03','DC04','DC16'].includes(code))return{kind:'conversa',action:`Converse com uma pessoa real sobre ${reference} e prepare perguntas antes.`,technique:'Pergunta aberta + escuta ativa',proof:'Registre as perguntas, duas respostas importantes e o que mudou na sua ideia.'};
 if(['DC07','DC11','CQ10'].includes(code))return{kind:'hábito',action:`Teste uma rotina curta que ajude você a avançar ${withPrep(reference,'em')} por alguns dias.`,technique:'Gatilho + ação pequena + registro',proof:'Mostre o registro dos dias e conte o que ajudou ou atrapalhou.'};
 if(['DC13','DC14','DC15','DC17','DC18','EX05'].includes(code))return{kind:'projeto',action:`Crie e teste uma versão pequena de algo ligado ${withPrep(reference,'a')}.`,technique:'Protótipo pequeno + teste real + melhoria',proof:'Mostre o que criou, o retorno de alguém e a melhoria que decidiu fazer.'};
 if(['CQ01','CQ04','CQ05','CQ06'].includes(code))return{kind:'dinheiro',action:`Levante valores reais e monte uma conta simples para ${reference}.`,technique:'Custo total + limite + alternativa',proof:'Envie sua conta, a fonte dos valores e a decisão que tomou.'};
 if(['CQ11','CQ12'].includes(code))return{kind:'decisão',action:`Tome uma decisão concreta sobre ${reference} e prepare o próximo passo.`,technique:'Critérios + escolha + plano de ação',proof:'Mostre sua escolha, os motivos e o próximo passo com data.'};
 return{kind:'pesquisa',action:`Investigue informações reais sobre ${reference} e compare o que encontrou.`,technique:'Fonte + comparação + conclusão',proof:'Envie as fontes, a comparação e sua conclusão.'};
}
function adaptPedagogy({age,classification,archetype}){
  const band=pedagogicalBand(age),reference=classification.natural_reference,activity=missionActivity(archetype.code,reference);
  ...
  if(band==='12')return{
    ...
    steps:[`Conte ao seu responsável o que você entendeu.`,activity.action,`Faça uma escolha ou melhoria pequena.`,activity.proof],
    technique:`${activity.technique}: faça uma parte de cada vez e use pelo menos uma informação real.`,
    example:example12,
    autonomy:`Você faz a comparação e toma a primeira decisão; o responsável ajuda a conferir informações e segurança.`,
    parent_support:`Seu responsável pode ajudar a encontrar duas fontes, fazer perguntas curtas e revisar o resultado, sem escolher por você.`,
    reflection:`O que você descobriu fazendo? Qual detalhe ajudou mais na sua escolha?`,
    evidence:`Envie uma prova simples do seu esforço. ${activity.proof}`,
  ...
  if(band==='14')return{
    ...
    steps:[`Defina o resultado desta missão.`,activity.action,`Compare duas ou três possibilidades e justifique seu caminho.`,activity.proof],
    technique:`${activity.technique}: aplique a técnica, registre seu raciocínio e explique por que sua decisão faz sentido.`,
    example:example14,
    autonomy:`Você pesquisa, organiza a comparação e propõe a decisão. O responsável entra como revisor e faz uma pergunta antes do envio.`,
    parent_support:`O responsável revisa os critérios e questiona uma suposição, mas não executa a pesquisa nem escreve a justificativa.`,
    reflection:`Qual critério mais influenciou sua decisão? Que nova informação faria você escolher diferente?`,
    evidence:`Mostre também o raciocínio por trás da sua escolha. ${activity.proof}`,
  ...
  return{
    age_band:band,
    intro:`Você vai investigar uma decisão importante com autonomia e produzir uma recomendação defensável sobre ${reference}, deixando claros limites, riscos e aquilo de que será preciso abrir mão.`,
    ...
    steps:[`Defina o resultado e os limites reais.`,activity.action,`Analise benefícios, custos, riscos e o que você abre mão.`,`Teste uma alternativa ou cenário diferente.`,activity.proof],
    technique:`${activity.technique} + cenários: sustente sua decisão com fontes, riscos e uma alternativa plausível.`,
    example:example17,
    autonomy:`Você define método, fontes e recomendação. O responsável valida limites familiares e questiona riscos, sem dirigir a análise.`,
    parent_support:`Seu responsável informa restrições reais da família e questiona a defesa final; pesquisa, análise e recomendação ficam com você.`,
    reflection:`O que você aceitou abrir mão? Qual suposição é mais frágil e como a decisão mudaria em outro cenário?`,
    evidence:`Apresente uma defesa completa, com limites e riscos. ${activity.proof}`,
    checklist:['Cite pelo menos três fontes','Separe fatos de suposições','Mostre pesos e um cenário alternativo','Defenda a recomendação e seus limites'],
    evidence_types:['TEXT',...archetype.allowed_evidence_types.filter(x=>['LINK','IMAGE','AUDIO'].includes(x))]
  };
}
export function buildJourney({classification,complexity,archetypes,parentGoals=[],interests=[],age=14,recentArchetypeCodes=/** @type {string[]} */([])}){
  const allowed=new Set(naturalCodes[classification.goal_type]||naturalCodes.other),desired=new Set(parentGoals.map(x=>skillMap[x]||String(x).toLowerCase()));const recent=new Set(recentArchetypeCodes);
  // O catálogo cobre 10–18 anos, enquanto um perfil Youth válido pode chegar a 21.
  // Use o teto do catálogo para elegibilidade e mantenha a pedagogia pelo age real.
  const catalogAge=Math.min(18,Math.max(10,Number(age)||14));
  const scored=archetypes.filter(a=>a.is_active&&allowed.has(a.code)&&catalogAge>=a.age_min&&catalogAge<=a.age_max).map(a=>{let score=5;const hits=(a.primary_skills||[]).filter(x=>desired.has(String(x).toLowerCase())).length;score+=hits*6;const interestSeed=interests.join('|').toLowerCase();if(interestSeed&&[...`${a.code}:${interestSeed}`].reduce((sum,char)=>sum+char.charCodeAt(0),0)%4===0)score+=2;if(recent.has(a.code))score-=8;const namedSkills=a.primary_skills.slice(0,2).map(x=>skillLabel[x]||x).join(' e '),why=hits?`Você vai avançar ${withPrep(classification.natural_reference,'em')} enquanto pratica ${namedSkills}, competências escolhidas pela sua família.`:`Você vai produzir uma decisão, conversa ou resultado real que ajuda a avançar ${withPrep(classification.natural_reference,'em')}.`;return{a,score,why}}).sort((x,y)=>y.score-x.score||x.a.code.localeCompare(y.a.code));
